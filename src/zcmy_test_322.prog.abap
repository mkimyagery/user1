...
*  SELECT * FROM zcm_tablo_2 INTO TABLE @DATA(gt_tablo_2) WHERE id IN @so_id AND yil EQ @p_yil.

*  DATA(lt_tablo_2) = gt_tablo_2[].
*  DELETE ADJACENT DUPLICATES FROM lt_tablo_2 COMPARING id.

  LOOP AT gt_tablo_2 INTO DATA(gs_tablo_2) WHERE id  IN so_id
                                             AND yil EQ p_yil
